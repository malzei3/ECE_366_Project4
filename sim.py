...
# created list to read non empty and un commented lines and clean/change excess char($, ' ', zero)
def readIn(s):
    text = ""
    with open(s, "r") as f:
        for line in f:
...
def main():
    ...
    good_in = False
    while(good_in == False):
        file__name = input("Please type file name, enter for default, or q to quit:\n")
    ...
    text = readIn(file__name)
    t = splitText(text)


    saveJumpLabel(text,labelIndex,labelName, labelAddr) # Save all jump's destinations
