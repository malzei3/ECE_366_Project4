...
# This Python program simulates the behavior of a valid MARS program
# and output simple statistics such as DIC,hit/miss rate of cache.
#
# NOTE: Since this is a simple Cache-sim *WITHOUT* replacement policy, any cache hit/miss will also
#       write into memory along with cache, that way if a block is replaced the memory still has most updated
#       values.
#       YOU need to implement LRU policy with the cache access.
...
def cache_simulate(Instruction, InstructionHex, debugMode):
    ...
    while not finished:
        ...
        if (fetch[0:32] == '00010000000000001111111111111111'):
            ...
            if (debugMode):
                print("PC =" + str(PC * 4) + " Instruction: 0x" + InstructionHex[PC] + " :" + "add $" + str(
            ...
            if (debugMode):
                print("PC =" + str(PC * 4) + " Instruction: 0x" + InstructionHex[PC] + " :" + "sub $" + str(
                    int(fetch[16:21], 2)) + ",$" + str(int(fetch[6:11], 2)) + ",$" + str(int(fetch[11:16], 2)))
            PC += 1
            Register[int(fetch[16:21], 2)] = Register[int(fetch[6:11], 2)] - Register[int(fetch[11:16], 2)]
            ...
            if (debugMode):
                print("PC =" + str(PC * 4) + " Instruction: 0x" + InstructionHex[PC] + " :" + "addi $" + str(
            ...
            imm = int(fetch[16:32], 2) if fetch[16] == '0' else -(65535 - int(fetch[16:32], 2) + 1)
            if (debugMode):
                print("PC =" + str(PC * 4) + " Instruction: 0x" + InstructionHex[PC] + " :" + "beq $" + str(
            ...
            imm = int(fetch[16:32], 2) if fetch[16] == '0' else -(65535 - int(fetch[16:32], 2) + 1)
            if (debugMode):
                print("PC =" + str(PC * 4) + " Instruction: 0x" + InstructionHex[PC] + " :" + "bne $" + str(
                    int(fetch[6:11], 2)) + ",$" + str(int(fetch[11:16], 2)) + "," + str(imm))
            PC += 1
            PC = PC + imm if Register[int(fetch[6:11], 2)] != Register[int(fetch[11:16], 2)] else PC
        elif (fetch[0:6] == '000000' and fetch[26:32] == '101010'):  # SLT
            if (debugMode):
                print("PC =" + str(PC * 4) + " Instruction: 0x" + InstructionHex[PC] + " :" + "slt $" + str(
                    int(fetch[16:21], 2)) + ",$" + str(int(fetch[6:11], 2)) + ",$" + str(int(fetch[11:16], 2)))
            PC += 1
            ...
            if (int(fetch[30:32]) % 4 != 0):
                ...
                exit()
            if (debugMode):
                print("PC =" + str(PC * 4) + " Instruction: 0x" + InstructionHex[PC] + " :" + "sw $" + str(
            ...
            if (debugMode):
                print("PC =" + str(PC * 4) + " Instruction: 0x" + InstructionHex[PC] + " :" + "lw $" + str(
...
def main():
    print("Welcome to ECE366 cache_sim, would you like to run simulator in debug mode ? ")
    ...
    cache_simulate(Instruction, InstructionHex, debugMode)
